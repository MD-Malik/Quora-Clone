...
async function signIn(req, res, next) {
  ...
  const decryptedPassword = encryptDecrypt.decryptPassword(
    req.body.password,
    response.password
  );
...
async function loginUsingGoogle(req, res) {
  ...
  let password = `${Math.random(100)}${username}${Math.random(
    100
  )}${useremail}${Math.random(100)}`;
  let userimage =
    "https://qsf.cf2.quoracdn.net/-4-images.new_grid.profile_default.png-26-688c79556f251aa0.png";
...
async function getDescription(req, res, next) {
  try {
    ...
    let response = await descriptionModel
      .find({ postId: mongoose.Types.ObjectId(req.params.postId) })
      .populate("postId");
    res.json(response);
  } catch (error) {
    res.status(500).json(error);
...
async function uploadImage(req, res, next) {
  ...
  let response1 = await tokenModel.findOne({ token });
  let response2 = await userModel.updateOne(
    { _id: response1.userid },
    { $set: { userimage } }
  );
  res.json({
    status: "success",
